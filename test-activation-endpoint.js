...
async function testActivationEndpoint() {
  try {
    ...
    if (!licenseResponse.ok) {
      ...
      throw new Error(
        `License fetch failed: ${licenseResponse.status} - ${error}`,
      );
    }
    ...
    console.log(
      `🔑 Using license: ${unusedLicense.key} (ID: ${unusedLicense.id})`,
    );
    console.log(
      `📊 License status: ${unusedLicense.isUsed ? "Used" : "Unused"}, Active: ${unusedLicense.isActive}`,
    );

    // Step 3: Test activation code generation
    console.log("⚡ Testing activation code generation...");
    const activationResponse = await fetch(
      `${BASE_URL}/api/license-keys/${unusedLicense.id}/generate-activation-code`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    console.log(
      `Response headers:`,
      Object.fromEntries(activationResponse.headers.entries()),
    );
