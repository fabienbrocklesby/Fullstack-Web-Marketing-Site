...
- **Trial entitlements expire.** Trials have a 14-day `expiresAt`. The app must display this expiry date. When expired, activation/refresh fails with `ENTITLEMENT_NOT_ACTIVE`.
- **Trial is retired on paid purchase.** When a customer purchases a paid plan, any active trial is immediately retired (status becomes `"expired"`, `expiresAt` set to now). The trial will no longer appear as usable.
...
| Term                           | Definition                                                                                                                  |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| **Entitlement**                | A customer's right to use the software (trial, subscription, or lifetime). Has a tier, status, and device limit.            |
| **Tier**                       | Feature level: `trial`, `maker`, `pro`, `education`, `enterprise`. Determines available features.                           |
| **Trial**                      | 14-day free trial (`tier: "trial"`). Has `expiresAt` set to 14 days from creation. Behaves like subscription (needs lease). |
...
```json
{
  ...
  "leaseToken": "<RS256 JWT>",
  "leaseExpiresAt": "2026-01-29T12:00:00.000Z",
  "entitlementExpiresAt": "2026-02-11T00:00:00.000Z"
}
```
...
| ---------------------- | -------------- | --------------------------------------------------- |
| `v`                    | number         | Must be `1`                                         |
| `type`                 | string         | Must be `"activation_package"`                      |
| `activationToken`      | string         | RS256 JWT for offline activation                    |
| `leaseToken`           | string         | RS256 JWT for lease period                          |
| `leaseExpiresAt`       | string         | ISO 8601 timestamp when lease expires               |
| `entitlementExpiresAt` | string \| null | ISO 8601 timestamp when entitlement expires (trial) |

> **Note:** `entitlementExpiresAt` is set for trial entitlements (14-day window) and `null` for subscription entitlements. App should use this to display trial countdown.
...
| ---------------------- | -------------- | --------------------------------------------------- |
| `v`                    | number         | Must be `1`                                         |
| `type`                 | string         | Must be `"lease_refresh_response"`                  |
| `leaseToken`           | string         | RS256 JWT for lease period                          |
| `leaseExpiresAt`       | string         | ISO 8601 timestamp when lease expires               |
| `entitlementExpiresAt` | string \| null | ISO 8601 timestamp when entitlement expires (trial) |

> **Note:** `entitlementExpiresAt` is present in every refresh response. App should update any stored trial countdown from this field.
...
| `entitlementExpiresAt` | After activation/refresh | When trial ends (null if not trial) |
...
### J.6 Lease Refresh Response (Decoded)
...
```json
{
  ...
  "type": "lease_refresh_response",
  "leaseToken": "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...",
  "leaseExpiresAt": "2026-02-05T12:00:00.000Z",
  "entitlementExpiresAt": "2026-02-11T00:00:00.000Z"
}
```
