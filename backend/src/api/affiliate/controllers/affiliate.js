...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
      ...
      try {
        ...
        if (!user) {
          return ctx.unauthorized(
            "You must be logged in to access affiliate data",
          );
        }
    ...
    async findOne(ctx) {
      ...
      const affiliate = await strapi.entityService.findOne(
        "api::affiliate.affiliate",
        id,
        {
          populate: ["purchases", "user"],
    ...
    async create(ctx) {
      try {
        ...
        if (!user) {
          return ctx.unauthorized(
            "You must be logged in to create an affiliate record",
          );
        }
        ...
        const existingAffiliates = await strapi.entityService.findMany(
          "api::affiliate.affiliate",
          {
            filters: {
              $or: [{ user: user.id }, { email: user.email }],
    ...
    async update(ctx) {
      ...
      const updatedAffiliate = await strapi.entityService.update(
        "api::affiliate.affiliate",
        id,
        {
    ...
    async delete(ctx) {
      ...
      const deletedAffiliate = await strapi.entityService.delete(
        "api::affiliate.affiliate",
        id,
      );
