...
function decryptDeactivationCode(encryptedCode, licenseKey) {
  try {
    ...
    if (parts.length !== 2) {
      throw new Error('Invalid encrypted code format. Expected format: {iv}:{encrypted}');
    }
...
if (args.length !== 2) {
  ...
  console.log('');
  console.log('Usage: node decrypt-deactivation-code.js <license-key> <encrypted-deactivation-code>');
  console.log('');
  console.log('Example:');
  console.log('  node decrypt-deactivation-code.js "ENTERPRISE-F4S8-8Y22-R88U" "a37587c13a0aff3a55fc5f2f291014a5:ff76d053e814da0644beb39431db4e93"');
  console.log('');
  console.log('The encrypted deactivation code should be in format: {iv}:{encrypted}');
  process.exit(1);
}
...
try {
  ...
  console.log('');
  console.log('💡 You can now use this deactivation code with the license key to deactivate the license.');
