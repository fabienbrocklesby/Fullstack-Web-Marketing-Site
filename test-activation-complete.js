...
async function testActivationWithLogin() {
  try {
    ...
    const unusedLicense = licenseData.licenseKeys.find(
    ...
    const activationResponse = await fetch(
      `${cmsUrl}/api/license-keys/${unusedLicense.id}/generate-activation-code`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${customerToken}`,
    ...
    if (activationResponse.ok) {
      ...
      console.log(
        "Activation code (first 50 chars):",
